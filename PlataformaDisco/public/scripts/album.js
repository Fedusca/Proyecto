...
function renderAlbum(album){
 ...
 function renderSongs(canciones) {
  ...
  canciones.forEach((cancion, index) => {
    ...
    console.log("Canción ID:", cancion._id); // Imprime el ID de la canción en la consola

    const deleteIcon = document.createElement('span');
    deleteIcon.innerHTML = '<i class="fa fa-trash text-red-500"></i>';
    deleteIcon.dataset.songId = cancion._id;
    deleteIcon.addEventListener('click', function() {
      deleteSong(currentAlbumId, this.dataset.songId); // Accede al ID de la canción desde el atributo de datos
    });
...
function deleteSong(albumId, songId) {
  console.log("Album ID:", albumId); // Imprime el ID del álbum en la consola
  console.log("Canción ID:", songId); // Imprime el ID de la canción en la consola
  
  
  axios.delete(`/Album/albumes/${albumId}/canciones/${songId}`)
    .then(() => {
      swal({
